"""Notebook 06 — SIGNAL DEFINITION on OWN-SPLICE series (L-013, D-020).
...
Identical to `qc_pair_minute_analysis.py` in every respect except the last
step: the same roll schedule, the same `build_continuous`, the same
`splice_audit` acceptance gate with the same A1 adjudication bound. Only the
final call differs — `pair_signal_report()` instead of `pair_minute_report()`
— because D-020 changes the SIGNAL DEFINITION and nothing else. Keeping the
data path byte-identical is what makes the Z0 reproduction gate meaningful:
if this run's Z0 grid does not match the banked notebook-02/03 grid for the
same pair, the pipeline moved and no Z0-vs-Z1 comparison is valid.

Variance-ratio blocks are not recomputed. They are functions of the residual
and the sampling interval only — no z-score enters them — so criterion (b) is
inherited unchanged from EXP-008/009/010/011.
...
`scripts/ingest_qc_signal_definition.py` for the parser.
...
`python scripts/build_qc_upload.py --driver signal`, which only rewrites
relative imports).
"""
...
from pair_minute_report import fmt, pair_signal_report
...
#
# D-020 fixes the scope to four pairs and the order they run in:
#   ("MES", "MYM") -> ("MES", "MNQ") -> ("MES", "M2K") -> ("ZF", "ZN")
# The first three are where L-013 is present (22.7-24.7% of crossings in the
# first 30 minutes); ZF-ZN is the NEGATIVE CONTROL, where it is absent (12.7%,
# flat profile). The remaining three Treasury pairs are deliberately not run:
# no redefinition of the entry signal raises a 0.166-0.664 bps effect through a
# 1.4-5.9 bps round trip.
PAIR = ("MES", "MYM")
...
class PairMinuteAnalysis(QCAlgorithm):
    ...
    def _run(self):
        ...
        self.results["S_PAIR"] = (
            ...
            f"rolls={'treasury' if a in TREASURIES else 'index'}|nb=06")
        # No roll timestamps or splice factors are passed: the D-020 battery has
        # no preflight block. The roll windows were adopted in report 02 §6.3 and
        # measured per pair in reports 02/03; re-emitting them would spend output
        # budget on numbers that cannot change with the signal definition.
        self.results.update(pair_signal_report(
            built[a][0], built[b][0], anchor=ANCHOR))
